...
def test_tangent_basis_is_orthonormal_and_orthogonal(key):
    xs = UniformOnSphere(3).sample(key, (8,))
    basis = jax.vmap(tangent_basis)(xs)
    identity = jnp.tile(jnp.eye(3), (8, 1, 1))
    assert basis.shape == (8, 4, 3)
...
def test_sphere_utils_jit_vmap_and_grad_smoke(key):
    ...
    xs = UniformOnSphere(2).sample(jr.fold_in(key, 1), (8,))
    ...
    ys = exp_fn(xs, vs)
    assert tangent_basis_fn(xs).shape == (8, 3, 2)
    assert log_fn(xs, ys).shape == (8, 3)
