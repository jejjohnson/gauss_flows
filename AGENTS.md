...
4. **Type checks** – `uv run --group typecheck ty check src/gauss_flows` (or `make typecheck`) must report no errors in changed files.

> **Common pitfall**: Running `ruff check src/gauss_flows/` instead of `ruff check .` misses lint errors in `tests/` and `scripts/`. CI runs `ruff check .` on the entire repo — always use `.` (repo root), not a subdirectory.
...
Examples:
- `feat: add SurVAE Slice surjection`
- `fix: correct log-det sign in HouseholderRotation`
- `docs: add 2D toy flow example notebook`
- `chore: bump ruff to 0.15.10`
