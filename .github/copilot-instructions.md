...
- **Project**: gauss_flows — JAX/FlowJax-based Gaussianization flows, SurVAE transforms, and information-theoretic measures.
- **Python**: 3.12+
- **Package Manager**: uv
- **Layout**: `src/` layout (`src/gauss_flows/`)
- **Testing**: pytest
...
make test        # Run tests (uv run pytest -v, with coverage)
...
**All four checks must pass before any commit.** CI runs them on the entire repo (`ruff check .`), not just `src/gauss_flows/`, so always run the commands below from the repo root.
...
uv run --group typecheck ty check src/gauss_flows
```

> **Common pitfall**: Running `ruff check src/gauss_flows/` instead of `ruff check .` misses import-sorting errors in `tests/`. The CI workflow runs `ruff check .`. Always use `.` (repo root), not a subdirectory.
...
| `src/gauss_flows/_src/transforms/` | Bijection / surjection / stochastic transforms |
| `src/gauss_flows/_src/flows.py` | Flow factories (`gaussianization_flow`, etc.) |
| `src/gauss_flows/_src/info_theory.py` | Entropy, MI, TC, KL, negentropy |
| `src/gauss_flows/_src/numpyro_compat.py` | `FlowDist` NumPyro distribution wrapper |
| `src/gauss_flows/_src/numpyro_guide.py` | `FlowGuide` variational guide |
| `tests/` | Test suite |
| `docs/` | Documentation (MkDocs) |
| `docs/notebooks/` | Jupytext percent-format example notebooks |
